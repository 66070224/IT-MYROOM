...
function Page() {
  ...
  const handleDelete = async (reservationId) => {
    try {
      const res = await fetch("/api/reservation/creative/deletereservationcreative", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg w-full justify-center">
      <div className="mb-5">
      <h2 className="text-xl font-semibold mb-4">My creative Reservations</h2>
          ...
          <tbody>
            {reservations.map((reservation) => (
              <tr key={reservation._id}>
                ...
                <td className="border px-4 py-2">{reservation.day}</td>
